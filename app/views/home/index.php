...
<?php foreach ($items as $item):?>        
          <div class="col-sm-6 col-md-10 col-md-offset-1">
            ...
            <table class="table">
              ...
              <p>
                    <a href="/mvc/public/home/show/<?php print($item["id"])?>" class="btn btn-primary btn-sm" role="button">View</a> 
                </p>
              </div>
            </div>
          </div>
...
<?php if( !preg_match('@/mvc/public/home/ended@', $_SERVER['REQUEST_URI']) == 1) :?>
<script>
  $(document).ready(function(){
    function update(){
    $.getJSON('/mvc/public/home/refresh').done(function(res) {
        $('#changeme').html(function(){
          ...
          for (var i=0;i<res.length;i++){
            ...
            '</table>'+
              ...
              '<p>Game started : <?php echo $item["started"]; echo $item["ended"]==null?"":" and Ended: ".$item["ended"] ?> </p>'+
              '<p>'+
                    '<a href="/mvc/public/home/show/'+res[i].id+'" class="btn btn-primary btn-sm" role="button">View</a> '+
                '</p>'+
              '</div>'+
            '</div>'+
          '</div>';
